...
# TODO: Refactor with track bar callbacks, rather then looping
...
def run(imgpath):
    ...
    while (1):
        ...
        cnts, new = cv2.findContours(wimg.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
...
help_info = '''
    Exploring the threshold impact on edge detection algorithm
    '''

parser = argparse.ArgumentParser(description=help_info, formatter_class=argparse.RawTextHelpFormatter)
parser.add_argument("--image", required=False, help="Image path", default=os.path.join(os.getcwd(), 'auto.jpg'))
